            ...
            <div class="col-12 col-lg-4 login card-login-x text-md-center">
                <div class="card card-login p-3 w-100">
                    <div class="card-body">
                        ...
                        <h5 class="card-title fw-bold">Ubah Sandi</h5>
                        <p class="card-text">Silakan masukkan sandi baru Anda</p>
                        ...
                        @endif
                        <form action="{{ url('confirm_forgot/') }}/{{$token}}" method="POST">
                            @method('POST')
                            @csrf
                            <div class="input-group mb-3">
                                <input type="password" class="form-control py-3 @error('password') is-invalid @enderror " id="exampleFormControlInput1" placeholder="Sandi Baru" name="password">
                                ...
                                <div class="input-group mb-3">                                    
                                    <input type="password" class="form-control py-3 @error('confirm_password') is-invalid @enderror " id="exampleFormControlInput1" placeholder="Konfirmasi Sandi Baru" name="confirm_password">
                                    <button id="btn-eye2" type="button" class="input-group-text bg-white"><i
                                        class="bi bi-eye"></i></button>
                                    ...
                                    class="btn bg-blue text-white w-100 mt-3">Kirim</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $("#btn-eye2").on('click', function() {
            if ($("input[name=confirm_password]").hasClass('show') == false) {
                $("input[name=confirm_password]").attr('type', 'text');
                $("input[name=confirm_password]").addClass('show');
            } else {
                $("input[name=confirm_password]").attr('type', 'password');
                $("input[name=confirm_password]").removeClass('show');
            }
        })
    });
</script>
